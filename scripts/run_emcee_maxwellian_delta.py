...
def logposterior(c):
    ...
    # If sample in prior range, evaluate
        ...
        # Draw catalog
        try:
            binaries,trials,sn,m,efficient = getPopRecursion(200,logR,a2_mean,a2_std,0.,"maxwellian",[10.**logv,10.**logv],0.9,efficiencyThreshold=1e-3,dist='delta')
        except RuntimeError:
...
if __name__=="__main__":
    ...
    initial_logR = np.random.random(nWalkers)+1.
    ...
    for i,result in enumerate(sampler.sample(initial_walkers,iterations=nSteps)):
        if i%10==0:
            np.save('output/emcee_samples_maxwellian_beta0.9_perfectAlignment_delta.npy',sampler.chain)
    np.save('output/emcee_samples_maxwellian_beta0.9_perfectAlignment_delta.npy',sampler.chain)
