...
def logposterior(c):
    ...
    if logR<logR_min or logR>logR_max or logv<logv_parallel_min or logv>logv_parallel_max or a2_mean<a2_mean_min or a2_mean>a2_mean_max or a2_std<a2_std_min or a2_std>a2_std_max:
        return -np.inf
    ...
    # If sample in prior range, evaluate
        ...
        # Draw catalog
        try:
            binaries,trials,sn,m,efficient = getPopRecursion(200,logR,a2_mean,a2_std,0.,"maxwellian",[10.**logv,10.**logv],0.9,efficiencyThreshold=3e-4,dist='delta')
        except RuntimeError:
...
if __name__=="__main__":
    ...
    initial_logR = np.random.random(nWalkers)+2.5
    initial_v = np.random.random(nWalkers)+0.5
    initial_a2_means = np.random.random(nWalkers)*0.3
    initial_a2_stds = np.random.random(nWalkers)*0.3+0.3
    initial_walkers = np.transpose([initial_logR,initial_v,initial_a2_means,initial_a2_stds])
    ...
    for i,result in enumerate(sampler.sample(initial_walkers,iterations=nSteps)):
        if i%10==0:
            np.save('output/emcee_samples_maxwellian_beta0.9_perfectAlignment_delta_wide.npy',sampler.chain)
    np.save('output/emcee_samples_maxwellian_beta0.9_perfectAlignment_delta_wide.npy',sampler.chain)
