...
mockDetections = h5py.File('/home/thomas.callister/CBC/state-of-the-field-gwtc2/input/o3a_bbhpop_inj_info.hdf','r')
...
def logposterior(c):
    ...
    # If sample in prior range, evaluate
        ...
        # Draw catalog
        try:
            binaries,trials,sn,m,efficient = getPopRecursion(400,1.0,a2_mean,a2_std,0.,"maxwellian",[10.**logv,10.**logv],0.9,efficiencyThreshold=1e-3,logNormal=True)
        except RuntimeError:
...
if __name__=="__main__":
    ...
    # Run
    nSteps = 10000
    sampler = mc.EnsembleSampler(nWalkers,dim,logposterior,threads=32)
    for i,result in enumerate(sampler.sample(initial_walkers,iterations=nSteps)):
        if i%50==0:
            np.save('/home/thomas.callister/CBC/state-of-the-field-gwtc2/output/emcee_samples_maxwellian_beta0.9_perfectAlignment_logNormal.npy',sampler.chain)
    np.save('/home/thomas.callister/CBC/state-of-the-field-gwtc2/output/emcee_samples_maxwellian_beta0.9_perfectAlignment_logNormal.npy',sampler.chain)
